/**
 * AI Generative Fill Web Worker
 *
 * Runs off-main-thread neural inpainting and generative synthesis on a
 * dedicated background thread via ONNX Runtime Web. Employs hardware-accelerated
 * WebGPU shaders with automatic WASM SIMD fallback.
 */
...
async function getWasmBlobPaths() {
  ...
  try {
    ...
    console.warn('[Generative Fill Worker] Failed to create blob paths for WASM:', err);
    return '/onnx/';
  }
}
...
/**
 * Stream model download with progress reporting.
 */
async function downloadModel(modelUrl) {
  self.postMessage({
    ...
    stage: 'loading-model',
    percent: 5,
    message: 'Connecting to generative AI model…',
  });

  const response = await fetch(modelUrl);
  if (!response.ok) {
    throw new Error(`Failed to download generative model from ${modelUrl} (HTTP ${response.status} ${response.statusText})`);
  }
...
/**
 * Initialize or return cached InferenceSession.
 */
async function getSession(modelUrl = MODEL_PATH) {
  ...
  self.postMessage({
    ...
    stage: 'preparing',
    percent: 80,
    message: 'Compiling neural generative network…',
  });
  ...
  if (typeof navigator !== 'undefined' && navigator.gpu) {
    try {
      ...
      console.warn('[Generative Fill Worker] WebGPU failed, falling back to WASM:', gpuErr?.message);
      session = null;
    }
  }
...
function maskToNCHW(maskData, size = 512) {
  ...
  for (let i = 0; i < pixelCount; i++) {
    const isMasked = maskData[i * 4 + 3] > 10 || maskData[i * 4] > 10;
    nchw[i] = isMasked ? 1.0 : 0.0;
  }
...
/**
 * Convert NCHW [1, 3, 512, 512] output float32 [0, 255] back to RGBA uint8.
 */
...
self.onmessage = async (e) => {
  const { type, id, modelUrl = MODEL_PATH, imageRgba, maskRgba } = e.data;

  if (type === 'generate-fill') {
    try {
      ...
      self.postMessage({
        ...
        stage: 'generating',
        percent: 92,
        message: 'Synthesizing new generative content…',
      });
      ...
      console.error('[Generative Fill Worker Error]:', err);
      self.postMessage({
        type: 'error',
        id,
        error: err?.message || 'Generative fill failed inside Web Worker.',
      });
    }
  }
};
